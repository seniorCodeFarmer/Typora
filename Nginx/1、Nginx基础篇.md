...
- 这一项的要求仅针对于那些对`linux`系统的防火墙设置规则不太清楚的，建议大家把防火墙都关闭掉，因为我们此次课程主要的内容是对`Nginx`的学习，把防火墙关闭掉，可以省掉后续`Nginx`学习过程中遇到的诸多问题。
