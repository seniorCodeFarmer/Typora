...
注意 ~后面不能加空格，括号可以取值
$符号用来获取server_name中括号中的具体值，$1用来获取第一个括号，$2用来获取第二个括号
```
...
No2：通配符在开始时匹配server_name成功（星号在www处的）

No3：通配符在结束时匹配server_name成功 （星号在com处的）
...
该指令用于设置Gzip压缩程度，级别从1-9，1表示压缩程度最低，效率最高，9刚好相反，压缩程度最高，但是效率最低最费时间。
...
针对不同种类客户端发起的请求，可以选择性地开启和关闭Gzip功能。如果匹配成功就关闭，匹配不成功就开启。
...
（5）使用configure来配置参数，需要将第一步查询出来的参数全部粘贴到configure后面

```
./configure --with-http_gzip_static_module
```

![image-20230227113818550](assets/image-20230227113818550.png)
...
![image-20230227114114842](assets/image-20230227114114842.png)
...
time：可以整数也可以是负数，指定过期时间，如果是负数，Cache-Control则为no-cache（走弱缓存），如果为正整数或0，则Cache-Control的值为max-age=time;
